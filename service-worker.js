...
const CACHE = 'child-learning-v32';
...
const VOICE = ['correct','greeting_afternoon','greeting_evening','greeting_late_morning','greeting_morning','greeting_night','greeting_noon','pet_bath','pet_hello','pet_hungry','pet_play','retry','trace_pass','trace_retry','trace_start',...POEM_LINES];
const PRECACHE = [
...
self.addEventListener('message', event => {
  ...
  const learningAudio = [...PINYIN, ...MATH, ...VOICE.map(x=>`./assets/voice/${x}.mp3`), './assets/voice/poem_yong_e_l1.wav', ...ENGLISH.flatMap(x => [
