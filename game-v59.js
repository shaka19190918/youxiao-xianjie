...
(function(){
  ...
  function homePetPanel(pet,due,next){
    ...
    let wish=due?'我们先把错题变成会做的题吧！':(next?'我准备好陪你挑战“'+next.title+'”啦！':'整座成长岛都亮起来啦！');
    if((dog.hu||0)<35)wish='我有点饿啦，完成学习赚钻石给我加餐吧！';
    else if((dog.hy||0)<35)wish='身上有点脏啦，攒好钻石帮我洗澡吧！';
    else if((dog.en||0)<35)wish='我想活动一下，今天通关后陪我玩吧！';
    const stats=[['🍖','饱腹',dog.hu||0,'#f4a63d'],['🛁','清洁',dog.hy||0,'#49b8e8'],['⚡','活力',dog.en||0,'#52bf78']];
    return `<aside class="g1-pet-house" aria-label="伙伴小屋"><header><strong>🐾 伙伴小屋</strong><span>${stage.n}</span></header><div class="g1-pet-main"><div class="g1-pet-picture">${petImageV6(dog.type,104,false)}</div><div><h2>${esc(pet.n)}</h2><p>${esc(wish)}</p><small>今天陪你完成 ${todayDone} 关</small></div></div><div class="g1-pet-stats">${stats.map(x=>`<div><span>${x[0]} ${x[1]} <b>${x[2]}</b></span><i><b style="width:${Math.max(0,Math.min(100,x[2]))}%;background:${x[3]}"></b></i></div>`).join('')}</div><div class="g1-pet-grow"><span><b>成长 ${dog.xp||0}</b>${nextStage?'再获得 '+Math.max(0,nextStage.xp-(dog.xp||0))+' 成长值进化':'已经完全进化'}</span><i><b style="width:${progress}%"></b></i></div><button onclick="showPage('dog')"><span>去照顾伙伴</span><b>进入小屋 →</b></button></aside>`;
  }
  ...
  function mountFloatControls(){
    let tray=document.getElementById('v64FloatControls');
    if(!tray){tray=document.createElement('div');tray.id='v64FloatControls';tray.setAttribute('aria-label','屏幕时间与声音控制');document.body.appendChild(tray)}
    const eye=document.getElementById('eyeBar'),sound=document.getElementById('v41Sound');
    if(eye&&eye.parentNode!==tray)tray.appendChild(eye);
    if(sound&&sound.parentNode!==tray)tray.appendChild(sound)
  }
